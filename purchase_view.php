...
To:<input type="text" style="width:223px; padding:4px;" name="d2" id="datepickerd" autocomplete="off">
 ...
 <i class="icon icon-search icon-large"></i> Search
	...
	$result2 = $db->prepare("SELECT * FROM purchases WHERE action='0' and   date BETWEEN '$d1' AND '$d2'  ORDER by transaction_id DESC");
	$result2->bindParam(':userid', $d2);
    $result2->execute();
    for($i=0; $row2 = $result2->fetch(); $i++){
    $invo=$row2['in_invoice'];
	$in_id=$row2['invoice_number'];
		...
		<td>
			<a href=" purchase_invoice_view.php?d1=<?php echo $row2['in_invoice'];?>"   title="Click to View" >
				 ...
				 <?php if($date==$now_date){ ?> 
				  <a href="#"   title="Click to Delete" id="<?php echo $invo; ?>" class="delbutton" >
...
//Built a url to send
var info = 'id=' + del_id;
 if(confirm("Sure you want to delete this Collection? There is Cancel undo!"))
		  {
